...
def predict_gt(east_detect, img_path, pixel_threshold, save_dir, quiet=False):
    """
    预测1000张有标准答案的图
    :return:
    """
    ...
    with Image.open(img_path) as im:
        ...
        im.save(img_save_path + '_act.jpg')
        quad_draw = ImageDraw.Draw(quad_im)
        txt_items = []
        for score, geo, s in zip(quad_scores, quad_after_nms,
                                 range(len(quad_scores))):
        ...
        quad_im.save(img_save_path + '_predict.jpg')
        if cfg.predict_write2txt and len(txt_items) > 0:
            with open(img_save_path[:-4] + '.txt', 'w') as f_txt:
                f_txt.writelines(txt_items)
...
if __name__ == '__main__':
    ...
    import glob
    img_path_list = glob.glob('./test_data/gt_set/*.jpg')
    for img_path in img_path_list[0:10]:
        predict_gt(east_detect, img_path, threshold, save_dir)
